...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const latestTransationPromise = Order.find({})
            ...
            .limit(4);
        const [thisMonthProducts, thisMonthUsers, thisMonthOrders, lastMonthProducts, lastMonthUsers, lastMonthOrders, productsCount, usersCount, allOrders, lastSixMonthOrders, categories, femaleUsersCount, latestTransation,] = await Promise.all([
        ...
        const orderMonthyRevenue = new Array(6).fill(0); //and all element is 0
...
export const getPieCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const grossnIncome = allOrders.reduce((prev, order) => prev + (order.total || 0), 0);
        const discount = allOrders.reduce((prev, order) => prev + (order.discount || 0), 0);
        const productionCost = allOrders.reduce((prev, order) => prev + (order.shippingCharges || 0), 0);
        const burnt = allOrders.reduce((prev, order) => prev + (order.tax || 0), 0);
        const marketingCost = Math.round(grossnIncome * (30 / 100));
        const netMargin = grossnIncome - discount - productionCost - burnt - marketingCost;
